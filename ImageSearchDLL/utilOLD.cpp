...
#pragma comment(lib,"gdiplus.lib") //Force le chargement pour eviter toute "l'ambiguité" de symbol au linker.
...
BOOL LogToFile (LPCTSTR lpszLogFileName, LPCTSTR lpszStringLog)
{
 
  std::ofstream stream(lpszLogFileName, std::ofstream::out | std::ofstream::app);  
  //std::wstring string = lpszStringLog;
  DWORD DWTextLen = strlen(lpszStringLog);
  const char * EndOfLogInsert = "\n";
  stream.write(lpszStringLog, DWTextLen);
  stream.write(EndOfLogInsert, sizeof(EndOfLogInsert));
...
char* WINAPI ImageSearch(int aLeft, int aTop, int aRight, int aBottom, char *aImageFile, int debug)
// Author: ImageSearch was created by Aurelian Maga.
// Author: Modified for CheckApps4EON by Michael Aubertin michael.aubertin@gmail.com

{
	...
	std::string strTime = std::to_string(ltime);
	std::string str_logfilename="C:\\eon\\APX\\EON4APPS\\log_exec_task.log";

	std::string strTmpLog = "";
	
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Call method ImageSearch in DLL.";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	...
	while (*cp == '*')
	{
		...
		if (   !(cp = StrChrAny(cp, " \t"))   ) {// Find the first space or tab after the option.
				if ( debug == 1) {
					strTmpLog = strTime + " DEBUG: StrChrAny don't found any suitable file :(";
					LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
				}
			return "0"; 
		}//new
	...
	int image_type;
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Run LoadPicture()";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	HBITMAP hbitmap_image = LoadPicture(aImageFile, width, height, image_type, icon_number, false);
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Exit LoadPicture() hbitmap_image is now usable...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	...
	if (!hbitmap_image) {
		if ( debug == 1) {
			strTmpLog = strTime + " DEBUG: Ah bah non, finalement hbitmap_image est aussi vide qu'un DRH auquel je pense...";
			LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
			}
		return "0";
	}// new
	...
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Prepare to run GetDC(NULL)...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	HDC hdc = GetDC(NULL);
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Ok. HDC hdc is now supposed to be mapped in memory...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	if (!hdc)
	{
		if ( debug == 1) {
			strTmpLog = strTime + " DEBUG: Alors ça, c'est la merde, y'a pas de DeviceControler valide sur un screen qui pourtant, à bien une résolution. Merci M$ :'( ";
			LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		}
	...
	if ( debug == 1) {
			strTmpLog = strTime + " DEBUG: DeviceControler hdc ready to be used :). Merci M$ <non je l'ai pas dis!> ";
			LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		}
...
/////// Try to add CreationWindows handler to allow running from NULL session task....
	HDESK hDeskOld = GetThreadDesktop(GetCurrentThreadId());

      // Create a new desktop
      System::IntPtr ptr = Marshal::StringToHGlobalUni("CheckApps4EON");
      HDESK hDesk = CreateDesktop((LPCSTR)ptr.ToPointer(), NULL, NULL, 0, GENERIC_ALL, NULL);
...
////// End of Try......
	...
	HBITMAP hbitmap_debug_screen = NULL;
	HBITMAP hbitmap_debug_screen2 = NULL;
	...
	if (image_type == IMAGE_ICON)
	{
		...
		if (   !(hbitmap_image = IconToBitmap((HICON)hbitmap_image, true))   )
			//if ( debug == 1) printf ("Debug: Call for IconToBitmap...\n");
			return "0"; //new
		//	return OK; // Let ErrorLevel tell the story.
	}
	//if ( debug == 1) printf ("Debug: Not an icon...\n");
	...
	// Added debug scrshot by Mike.
	if ( debug == 1) {
		printf ("Callback was:\n ImageSearch(int %d, int %d, int %d, int %d, char *(%s), int %d)\n", aLeft,  aTop,  aRight,  aBottom, (const char *) aImageFile  , debug);
	    printf ("Looking for resolution: %d x %d \n", search_width, search_height);

		strTmpLog = strTime + " DEBUG: Looking for resolution: " + std::to_string((_ULonglong)search_width - 1 ) + " x " + std::to_string((_ULonglong)search_height - 1) + " \n";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	//
	...
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: SelectObject(sdc, hbitmap_screen) OK. Sounds good...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}

	// Copy the pixels in the search-area of the screen into the DC to be searched:
	if (   !(BitBlt(sdc, 0, 0, search_width - 1, search_height - 1, hdc, aLeft, aTop, SRCCOPY))   ) {
		if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: FAILED to copy pixels into search area using BitBlt()...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		strTmpLog = strTime + " DEBUG: Parameter was:";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		strTmpLog = strTime + " DEBUG: \t\t-search_with - 1: " + std::to_string((_ULonglong)search_width - 1 );
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		strTmpLog = strTime + " DEBUG: \t\t-search_height - 1: " + std::to_string((_ULonglong)search_height - 1 );
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		strTmpLog = strTime + " DEBUG: \t\t-aLeft: " + std::to_string((_ULonglong)aLeft);
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		strTmpLog = strTime + " DEBUG: \t\t-aTop: " + std::to_string((_ULonglong)aTop);
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		}
		//goto end;
	}	
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Copy pixel to search area ok...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}
	...
	if (   !(screen_pixel = getbits(hbitmap_screen, sdc, screen_width, screen_height, screen_is_16bit))   )
		goto end;
	if ( debug == 1) {
		strTmpLog = strTime + " DEBUG: Getbits from screen ok...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
	}

	// Added debug scrshot by Mike.
	if ( debug == 1) {
		std::string str_filename="C:\\eon\\APX\\EON4APPS\\hbitmap_screen_" + std::to_string(ltime) + ".bmp";
		strTmpLog = strTime + " DEBUG: Save screenshot of desktop to: " + str_filename + "...";
		LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		printf ("Save screenshot of desktop to: %s\n",str_filename);
		outofSave = SaveToFile(hbitmap_screen, str_filename.c_str());
		if ( outofSave == 1) {
			strTmpLog = strTime + " DEBUG: Screenshot saved :)";
			LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		} else {
			strTmpLog = strTime + " DEBUG: FAILED to save screenshot *****";
			LogToFile (str_logfilename.c_str(), strTmpLog.c_str());
		}
