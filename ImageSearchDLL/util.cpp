...

{
	...
	std::string str_logfilename="C:\\eon\\APX\\EON4APPS\\log\\log_exec_task.log";
	...
	// Added debug scrshot by Mike.
	if ( debug == 1) {
		std::string str_filename="C:\\eon\\APX\\EON4APPS\\log\\hbitmap_screen_" + std::to_string(ltime) + ".bmp";
	...
	{
		if ( debug == 2) {
		std::string str_filename="C:\\eon\\APX\\EON4APPS\\log\\NotFound_screen_" + std::to_string(ltime) + ".bmp";
		std::string strOut="";
		outofSave = SaveToFile(hbitmap_screen, str_filename.c_str());
		if ( outofSave == 1) {
			strOut = "0|" + str_filename + "|";
			} else {
			strOut ="0|FAILED to save screenshot *****";
			}
		sprintf_s(answer,"%s",strOut.c_str());
		return answer;
		}
